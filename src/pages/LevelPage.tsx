...
export function LevelPage() {
  ...
  const tracks: SkillTrack[] = useMemo(() => {
    const learned = profile.learnedCount || 186;
    return [
      {
        key: 'literacy',
        label: '识字',
        emoji: '🌱',
        value: pct(skills.literacy || 78),
        description: `已掌握 ${learned} 字`,
        nextGoal: '再学 14 字升到 Lv.7 山林探险家',
        color: 'mint',
      },
      {
        key: 'reading',
      ...
      },
      {
        ...
        label: '表达',
        emoji: '💬',
        value: pct(skills.expression || 45),
        description: '能用 3 个字说一句话',
      ...
      <TopBar
        ...
        right={
          <div className="level-summary">
            ...
            <span className="level-summary__sep">/ {totalChars}</span>
          </div>
        }
      />
      ...
      <main className="page-level__main">
        ...
        <section className="level-hero">
          ...
          <div className="level-hero__copy">
            <div className="level-hero__name">{profile.displayName}</div>
            <div className="level-hero__sub">
              已掌握 <b>{mastered}</b> 个字 · 累计学习 <b>{profile.learnedCount || 220}</b> 字
            </div>
          </div>
        </section>

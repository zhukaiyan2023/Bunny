...
export function HomePage() {
  ...
  const learnedCount = useMemo(() => profile.learnedCount ?? Object.keys(profile.mastery).length, [profile]);
  ...
  return (
    <div className="home-page">
      <TopBar title="Bunny 世界" subtitle="今天也和 Bunny 一起读、玩、发现汉字" showBack={false} right={(
      ...
      <main className="home-page__main">
        <section className="home-hero" aria-label="Bunny 欢迎区">
          ...
          <div className="home-hero__speech"><div className="home-hero__speech-title">今天想去哪里？</div><div className="home-hero__speech-text">选一个地方，Bunny 就带你出发。</div></div>
          <div className="home-hero__bunny"><Bunny pose="cheering" size={320} showBackpack /></div>
          <div className="home-hero__stats"><div className="home-stat"><span className="home-stat__value">{profile.badges.length}</span><span className="home-stat__label">我的勋章</span></div><div className="home-stat"><span className="home-stat__value">{profile.streakDays}</span><span className="home-stat__label">连续学习</span></div><div className="home-stat"><span className="home-stat__value">12</span><span className="home-stat__label">今日分钟</span></div></div>
        </section>

        <section className="home-islands" aria-label="学习入口">
          <div className="home-islands__heading"><div><div className="home-islands__eyebrow">选择你的下一步</div><h1>四个世界，一个学习旅程</h1></div><button className="home-islands__more" type="button" onClick={() => navigate('/daily')}>今日学习计划 →</button></div>
          <div className="home-islands__grid">
            {ISLANDS.map((island) => (
              <button key={island.id} type="button" className={`home-island home-island--${island.theme}`} onClick={() => navigate(island.to)}>
                ...
                <div className="home-island__footer"><span>{island.id === 'characters' && `已探索 ${learnedByIsland.plants ?? 0} 个字`}{island.id === 'game' && '探索 10 个冒险站点'}{island.id === 'story' && '推荐：1 本分级绘本'}{island.id === 'house' && `已有 ${profile.badges.length} 枚勋章`}</span><span>进入 →</span></div>
              </button>
            ))}
          </div>
        </section>
